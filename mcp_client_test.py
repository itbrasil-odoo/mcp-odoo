...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cliente de teste para o servidor Odoo MCP"
    )
    parser.add_argument(
        "--url", default="http://localhost:8080", help="URL do servidor MCP"
    )
    ...
    model_info_parser = subparsers.add_parser(
        "model-info", help="Obter informações de um modelo"
    )
    ...
    search_parser.add_argument(
        "domain",
        help='Domínio de busca em formato JSON (ex: \'[["is_company", "=", true]]\')',
    )
    ...
    method_parser.add_argument(
        "--args",
        help='Argumentos posicionais em formato JSON (ex: \'[["is_company", "=", true]]\')',
    )
    method_parser.add_argument(
        "--kwargs", help="Argumentos nomeados em formato JSON (ex: '{\"limit\": 10}')"
    )

    # Comando para buscar funcionários
    employee_parser = subparsers.add_parser(
        "search-employee", help="Buscar funcionários"
    )
    employee_parser.add_argument("name", help="Nome (ou parte do nome) a ser buscado")
    employee_parser.add_argument(
        "--limit", type=int, default=20, help="Número máximo de resultados"
    )
    ...
    holiday_parser.add_argument(
        "--employee-id", type=int, help="ID do funcionário (opcional)"
    )
