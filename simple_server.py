        ...
        if self.path == "/mcp/resource":
            ...
            try:
                ...
                if not resource:
                    ...
                    self.wfile.write(
                        json.dumps({"error": "Missing 'resource' parameter"}).encode()
                    )
                    return
                ...
                try:
                    if resource == "odoo://models":
                        ...
                        try:
                            ...
                            model_info = odoo_client._execute(
                                "ir.model", "read", model_ids, ["name", "model"]
                            )[0]
                        ...
                        try:
                            ...
                            record = odoo_client.read_records(
                                model_name, [record_id_int]
                            )
                        ...
                        try:
                            ...
                            results = odoo_client.search_read(
                                model_name, domain_list, limit=limit
                            )
                    ...
                    self._set_headers(500)
                    self.wfile.write(
                        json.dumps({"error": str(e), "resource": resource}).encode()
                    )
                ...
                self._set_headers(500)
                self.wfile.write(
                    json.dumps({"error": f"Server error: {str(e)}"}).encode()
                )
        else:
            self._set_headers(404)
            self.wfile.write(json.dumps({"error": "Not found"}).encode())
